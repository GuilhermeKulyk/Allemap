                        ...
                        <div class="row justify-content-center">
                            <div class="col-md-9">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                    @if ($meal->exists)
                                        <form  id='form-edit-meal' action="{{ route('meal.update', ['meal' => $meal->id]) }}">
                                    @else 
                                        <form id='form-create-meal' action="{{ route('meal.store') }}">
                                    @endif
                                            @csrf
                                            <!-- Campo para adicionar ou editar o nome do alimento -->
                                            <div class="form-group mb-3">
                                                <label for="name" class="form-label">{{ __('messages.words.name') }}</label>
                                                <input type="text" class="form-control" id="name" name="name" value="{{ isset($meal->id) ? $meal->name : (old('name') ?? '') }}">
                                            ...
                                            <div class="form-group mb-3">
                                                ...
                                                <select class="form-select" id="category_id" name="category_id">
                                                    ...
                                                    @foreach ($userFoodCategories as $category)
                                                        <option value="{{ $category->id }}" {{ $meal->mealCategory && $meal->mealCategory->id === $category->id ? 'selected' : '' }}>
